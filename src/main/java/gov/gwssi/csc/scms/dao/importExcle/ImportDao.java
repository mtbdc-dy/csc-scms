...
public class ImportDao extends BaseDAO {
    ...
    public List getList(String pro,String univ) {
        ...
        stringBuilder.append("SELECT t.FILENAME,t.CNT,t.STATE,t.CREATEBY,t.CREATED,t.error from SCMS_IMPORT_LOG t where 1 = 1");
        if(!"".equals(pro)){
...
//        }
    ...
    private List<String> getValueAndCheck(Vector<String> data,
        ...
        if ("".equals(cicNo)) {
            stringList.add("第"+(i+1)+"行，CSC登记号不能为空");
        }else{
        if(student==null){
            stringList.add("第"+(i+1)+"行，CSC登记号不存在");
        }
        }
        if ("".equals(elcregisteNo)) {
            stringList.add("第"+(i+1)+"行，学籍电子注册号不能为空");
        }
